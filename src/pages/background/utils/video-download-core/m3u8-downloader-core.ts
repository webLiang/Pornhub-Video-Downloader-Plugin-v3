...
class M3U8Downloader {
  ...
  private customHeaders: Record<string, string> = {}; // Custom headers injected via declarativeNetRequest (e.g. Origin, Referer)
  private activeRuleIds: number[] = []; // Active declarativeNetRequest session rule IDs

  // Headers that are "forbidden" in fetch() but CAN be set via declarativeNetRequest
  private static readonly DNR_HEADER_WHITELIST = [/^Origin$/i, /^Referer$/i, /^Cookie$/i];
  ...
  /**
   * Register declarativeNetRequest session rules to set custom headers (Origin, Referer, etc.)
   * for the given URLs. These headers are "forbidden" in fetch() but can be set at the
   * Chrome network stack level via declarativeNetRequest.
   *
   * Flow (same as Video DownloadHelper):
   *   1. Filter customHeaders → keep only DNR_HEADER_WHITELIST entries
   *   2. Extract unique base-URL patterns from the URL list (directory + wildcard)
   *   3. Register session rules: { modifyHeaders: "set Origin=xxx", urlFilter: pattern }
  ...
  private async registerHeaderRules(urls: string[]): Promise<void> {
    ...
    // Only keep forbidden headers that need declarativeNetRequest
    const requestHeaders: any[] = [];
    for (const [header, value] of Object.entries(this.customHeaders)) {
      if (M3U8Downloader.DNR_HEADER_WHITELIST.some(re => re.test(header))) {
        requestHeaders.push({ operation: 'set', header, value });
      }
    }
    if (requestHeaders.length === 0) {
      console.warn('[M3U8Downloader] registerHeaderRules: no whitelisted headers found in', this.customHeaders);
      return;
    }

    // Extract unique base URL patterns (directory-level wildcard, same as VDH's $i())
    const patterns = new Set<string>();
    for (const url of urls) {
      try {
        patterns.add(new URL('.', url).href + '*');
      } catch {
        patterns.add(url);
      }
    }
    ...
    for (const pattern of patterns) {
      ...
      rules.push({
        ...
        // NOTE: Do NOT specify resourceTypes — Service Worker fetch() requests
        // are NOT classified as "xmlhttprequest" (that type is for page/content-script).
        // Omitting resourceTypes matches ALL resource types; the urlFilter is specific enough.
        condition: { urlFilter: pattern },
      });
    }

    try {
      console.log('[M3U8Downloader] Registering DNR rules:', JSON.stringify(rules, null, 2));
      await chrome.declarativeNetRequest.updateSessionRules({ addRules: rules });
      this.activeRuleIds.push(...newRuleIds);
      console.log(`[M3U8Downloader] ✅ Registered ${rules.length} DNR rules, activeRuleIds:`, [...this.activeRuleIds]);

      // Verify: query all session rules to confirm they exist
      const existingRules = await chrome.declarativeNetRequest.getSessionRules();
      console.log(`[M3U8Downloader] 📋 Verify: ${existingRules.length} session rules active`, existingRules);
    } catch (e) {
      console.error('[M3U8Downloader] ❌ Failed to register declarativeNetRequest rules:', e);
    }
  }

  /**
   * Remove all active declarativeNetRequest session rules.
   * Called on download complete / error / cancel / destroy.
   */
  private async cleanupHeaderRules(): Promise<void> {
    if (this.activeRuleIds.length === 0) return;
    const idsToRemove = [...this.activeRuleIds];
    this.activeRuleIds = [];
    try {
      await chrome.declarativeNetRequest.updateSessionRules({ removeRuleIds: idsToRemove });
      this.log(`Removed ${idsToRemove.length} declarativeNetRequest rules`);
    } catch (e) {
      this.error('Failed to remove declarativeNetRequest rules:', e);
    }
  }
  ...
  private ajax(options: AjaxOptions): void {
    ...
    // Build fetch options with custom headers using multiple strategies:
    //   1. Direct headers in fetch() — extension SW may allow some "forbidden" headers
    //   2. `referrer` + `referrerPolicy` fetch option — standard way to set Referer
    //   3. declarativeNetRequest rules (registered in start/getM3U8) — network-level backup
    const fetchInit: RequestInit = {
      signal: abortController.signal,
    };
    if (Object.keys(this.customHeaders).length > 0) {
      fetchInit.headers = { ...this.customHeaders };
      // Set Referer via the fetch referrer option (may work even when header is forbidden)
      if (this.customHeaders['Referer']) {
        fetchInit.referrer = this.customHeaders['Referer'];
  ...
  private reset(): void {
    ...
    // Reset custom headers and cleanup declarativeNetRequest rules
    this.customHeaders = {};
  ...
  public start(url: string, options: StartOptions = {}): void {
    ...
    // Register declarativeNetRequest rules for the m3u8 URL before fetching,
    // so Origin/Referer headers are injected at the network level.
    if (Object.keys(this.customHeaders).length > 0) {
      this.registerHeaderRules([url]).then(() => this.getM3U8());
    } else {
      this.getM3U8();
    }
  }
  ...
  private getM3U8(): void {
    ...
    this.ajax({
      ...
      success: (m3u8Str: ArrayBuffer | string) => {
        ...
        // Parse AES config first (needed for URL collection below)
        let hasAES = false;
        ...
        // Register declarativeNetRequest rules for ALL segment URLs (+ AES key URL)
        // so that Origin/Referer headers are injected at the network level for every fetch.
        ...
        if (Object.keys(this.customHeaders).length > 0) {
          ...
          if (this.aesConf.uri) allUrls.push(this.aesConf.uri);
          this.registerHeaderRules(allUrls).then(startDownloads);
        } else {
          startDownloads();
        }
      },
  ...
  private triggerError(message: string): void {
    this.downloading = false;
    this.stopAutoRetry();
    this.cleanupHeaderRules(); // Remove declarativeNetRequest rules on error
  ...
  private triggerComplete(): void {
    ...
    this.mediaFileList = [];
    this.opfsWriteQueue.clear();
    this.cleanupHeaderRules(); // Remove declarativeNetRequest rules on complete
